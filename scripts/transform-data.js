...
// src/macintosh.src.mjs → macintosh.mjs
const LIB_TEMPLATE = fs.readFileSync('./src/macintosh.src.mjs', 'utf8');
const createLib = template(LIB_TEMPLATE, TEMPLATE_OPTIONS);
const libCode = createLib(require('./export-data.js'));
fs.writeFileSync('./macintosh.mjs', libCode);

// src/macintosh.d.ts → macintosh.d.ts
const TYPES_TEMPLATE = fs.readFileSync('./src/macintosh.d.ts', 'utf8');
const createTypes = template(TYPES_TEMPLATE, TEMPLATE_OPTIONS);
const typesCode = createTypes(require('./export-data.js'));
fs.writeFileSync('./macintosh.d.ts', typesCode);
